...
void RUN()
{
    while (ros::ok())
    {
        std::this_thread::sleep_for(std::chrono::milliseconds(10));  // 控制循环频率，避免过高的 CPU 占用
        // 与回调交换
        // if(detector_thread != busy)
        {
            sensor_msgs::PointCloud2 radar_msgs_copy;
            radar_msgs_copy.width = -1;
            ...
            if (!event_buffer.empty())
            {
                ...
                // 数据交换
                {
                    ...
                    if (!radar_buffer.empty()){
                        radar_msgs_copy = radar_buffer.front();
            ...
            if (radar_msgs_copy.width != -1)
            {
                DopplerEstimation(radar_msgs_copy);    
                event_detector_->event_stream.push_back(event_msgs_copy);
            }       
            ...
            if (event_detector_->Detector())
            {
                ...
                // 临时添加
                {
                    std::fstream detector_file("/media/hao/hao2/228/data/comb-3/vel/detector_vel",std::ios::out | std::ios::app);
                ...
                estimator_->Estimate2(twist2);
            }

            LOG(ERROR) << "Done" << std::endl;
        }      
    }
}
...
void RadarCallback(const sensor_msgs::PointCloud2::ConstPtr radar_msgs_ptr)
{
    ...
    // 数据交换
    {
        ...
        radar_buffer.push_back(radar_msgs_copy);
    }
}
