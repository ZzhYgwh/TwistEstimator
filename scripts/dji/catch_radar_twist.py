...
def publish_twist_data(timestamp, linear_velocity, angular_velocity):
    ...
    rospy.loginfo(f"Published Twist Detector message at timestamp: {timestamp}")
    ...
    while not rospy.is_shutdown():
        ...
        if pose_data_store.twist_queue and pose_data_store.pose_queue:
            ...
            if attitude_msg:
                ...
                # 将结果保存为 TUM 格式
                with open('/home/hao/Desktop/radar-event-new/src/TwistEstimator/output/detector2.tum', 'a') as f:
...
def main():
    rospy.init_node('catch_radar_twist', anonymous=True)

    # 创建 PoseDataStore 实例，用于存储和管理位姿数据
    pose_data_store = PoseDataStore(max_size=100000000)

    # 创建订阅者
    rospy.Subscriber('/radar/twist', TwistWithCovarianceStamped, twist_callback, pose_data_store)
