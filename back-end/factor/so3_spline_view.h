...
class So3SplineView {
 public:
  ...
  static SO3 EvaluateRp(const int64_t time_ns,
                        ...
                        JacobianStruct* J = nullptr) {
    std::pair<double, size_t> ui = splne_meta.computeTIndexNs(time_ns);
    // LOG(ERROR) << "ui = " << ui.first << ", " << ui.second << std::endl;
    size_t s = ui.second;
    double u = ui.first;
    ...
    // LOG(ERROR) << "J is " << ((J != nullptr)? "existing": "missing") << std::endl; 

    if (J) {
      J->start_idx = s;
      // LOG(ERROR) << "J->start_idx = " << J->start_idx << std::endl; 
    }
...
//    computeBlendingMatrix<SplineOrder, double, true>();