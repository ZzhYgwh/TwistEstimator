...
camera_twist = pd.read_csv('../output/twist.tum', header=None, delim_whitespace=True) # twist
detector = pd.read_csv('./river.tum', header=None, delim_whitespace=True) # detector2 ./river.tum
...
estimator_angular_velocity[:, 1] = -1.0 * temp
...
detector_angular_velocity[:, 1] = -1.0 * temp
...
camera_linear_velocity[np.abs(camera_linear_velocity) > 5] = 0
detector_linear_velocity[np.abs(detector_linear_velocity) > 5] = 0
estimator_linear_velocity[np.abs(estimator_linear_velocity) > 5] = 0
