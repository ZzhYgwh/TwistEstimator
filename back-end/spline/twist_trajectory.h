...
#include "rd6_spline.h"
#include "trajectory.h"


// enum SensorType {
//   RadarSensor = 0,  //  qurey pose and EP
//   RadarSensor,
//   CameraSensor,
//   RadarSensor,
// };
...
// enum SensorType {
//   RadarSensor = 0,  // main cooridnate
//   EventSensor = 1
// };

// class TrajectoryManager;

class Twist_Trajectory : public Rd6Spline<SplineOrder, double> {
 EIGEN_MAKE_ALIGNED_OPERATOR_NEW 
 public:
  typedef std::shared_ptr<Twist_Trajectory> Ptr;
  ...
  Twist_Trajectory(double time_interval, double start_time = 0)
      : Rd6Spline<SplineOrder, double>(time_interval * S_TO_NS,
                                       start_time * S_TO_NS),
        data_start_time_(-1),
        active_time_(-1),
    ...
    this->extendKnotsTo(start_time * S_TO_NS,
                        Vec3(0, 0, 0),
                        Vec3(0, 0, 0));
    // LOG(ERROR) << "spline inilize size: " << this->getKnotSize() << std::endl;
    // LOG(ERROR) << "maxTimeNs() = " << this->maxTime(RadarSensor) << std::endl;
    ExtrinsicParam EP_StoI;
    EP_StoI_[RadarSensor] = EP_StoI;
  }
  ...
  Eigen::Vector3d GetLiearVelWorld(const double timestamp) const {  
    auto& Ep = EP_StoI_.at(RadarSensor);
    return this->LinearVelWorld(timestamp * S_TO_NS + Ep.t_offset_ns);
  }

  // Eigen::Vector3d GetTransVelWorld(const double timestamp) const {
  Eigen::Vector3d GetLinearAccWorld(const double timestamp) const {
    auto& Ep = EP_StoI_.at(RadarSensor);
    return this->LinearAccWorld(timestamp * S_TO_NS + Ep.t_offset_ns);
  }

  Eigen::Vector3d GetLinearJerkWorld(const double timestamp) const {
    auto& Ep = EP_StoI_.at(RadarSensor);
    return this->LinearJerkWorld(timestamp * S_TO_NS + Ep.t_offset_ns);
  }

  Eigen::Vector3d GetAngularVelWorld(const double timestamp) const {
    auto& Ep = EP_StoI_.at(RadarSensor);
    return this->AngularVelWorld(timestamp * S_TO_NS + Ep.t_offset_ns);
  }

  Vec6 GetRadarVel(const double timestamp) const {
    return GetSensorVel(timestamp, EP_StoI_.at(RadarSensor));
  }

  Vec6 GetEventVel(const double timestamp) const {
    return GetSensorVel(timestamp, EP_StoI_.at(EventSensor));
  }
  ...
  void SetForcedFixedTime(double time) {
    if (time < minTime(RadarSensor))
      forced_fixed_time_ = minTime(RadarSensor);
    else
 ...
 protected:
  Vec6 GetSensorVel(const double timestamp,
