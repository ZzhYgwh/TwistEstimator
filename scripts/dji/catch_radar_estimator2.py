...
def publish_twist_data(timestamp, linear_velocity, angular_velocity):
    ...
    rospy.loginfo(f"Published Twist Estimate message at timestamp: {timestamp}")
...
def attitude_callback(attitude_msg, pose_data_store):
    ...
    with pose_data_store.lock:
        pose_data_store.add_pose(timestamp, quaternion)
...
def radar_twist_callback(pose_data_store):
    ...
    while not rospy.is_shutdown():
        if not local_buffer:
            with pose_data_store.lock:
                N = 20
                for _ in range(min(N, len(pose_data_store.twist_queue))):
                    twist_timestamp, twist_msg = pose_data_store.twist_queue[0]
                    attitude_msg = pose_data_store.get_closest_pose(twist_timestamp)

                    if attitude_msg:
                        pose_data_store.twist_queue.popleft()
                        local_buffer.append((twist_timestamp, twist_msg, attitude_msg))
                    else:
                        break

        while local_buffer:
            twist_timestamp, twist_msg, attitude_msg = local_buffer.pop(0)
            ...
            rotated_linear_velocity.z *= -1.0
            rotated_angular_velocity = angular_velocity

            # 缓冲写入内容
            write_buffer.append(
                f"{twist_timestamp} {rotated_linear_velocity.x} {rotated_linear_velocity.y} {rotated_linear_velocity.z} "
        ...
        # 判断是否应该写入文件
        if len(write_buffer) >= max_buffer_size or (time.time() - last_write_time) > write_interval:
            rospy.loginfo(f"save data len: {len(write_buffer)}")
            with open('/home/hao/Desktop/radar-event-new/src/TwistEstimator/output/estimate.tum', 'a') as f:
...
def main():
    rospy.init_node('twist_attitude_processor', anonymous=True)
