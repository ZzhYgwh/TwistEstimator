...
class Trajectory : public Se3Spline<SplineOrder, double> {
 public:
  ...
  SE3d GetRadarPose(const double timestamp) const {
    return GetSensorPose(timestamp, EP_StoI_.at(RadarSensor));
  }

  SE3d GetEventPose(const double timestamp) const {
    return GetSensorPose(timestamp, EP_StoI_.at(EventSensor));
  }
