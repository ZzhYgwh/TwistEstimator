...
camera_twist = pd.read_csv('twist.tum', header=None, delim_whitespace=True) # twist twist_output
detector = pd.read_csv('detector2.tum', header=None, delim_whitespace=True) # detector2 ./river.tum
estimator = pd.read_csv('detector2.tum', header=None, delim_whitespace=True) # estimate.tum
...
# detector_linear_velocity[:, 0] = 1.0 * detector_linear_velocity[:, 1]
# detector_linear_velocity[:, 1] = temp
# detector_linear_velocity[:, 2] = -1.0 * detector_linear_velocity[:, 2]

# temp = 1.0 * estimator_linear_velocity[:, 0].copy()
# estimator_linear_velocity[:, 0] = 1.0 * estimator_linear_velocity[:, 1]
...
# temp = detector_angular_velocity[:, 1].copy()
# detector_angular_velocity[:, 1] = detector_angular_velocity[:, 2]
# detector_angular_velocity[:, 2] = temp
...
temp = -1.0 * detector_angular_velocity[:, 2].copy()
detector_angular_velocity[:, 2] = -1.0 * detector_angular_velocity[:, 1]
detector_angular_velocity[:, 1] = temp
# detector_angular_velocity[:, 0] = -1.0 * detector_angular_velocity[:, 0]
temp = -1.0 * detector_angular_velocity[:, 0].copy()
detector_angular_velocity[:, 0] = -1.0 * detector_angular_velocity[:, 1]
detector_angular_velocity[:, 1] = temp

temp = -1.0 * estimator_angular_velocity[:, 2].copy()
estimator_angular_velocity[:, 2] = -1.0 * estimator_angular_velocity[:, 1]
estimator_angular_velocity[:, 1] = temp
temp = -1.0 * estimator_angular_velocity[:, 0].copy()
estimator_angular_velocity[:, 0] = -1.0 * estimator_angular_velocity[:, 1]
estimator_angular_velocity[:, 1] = temp
# estimator_angular_velocity[:, 0] = -1.0 * estimator_angular_velocity[:, 0]

# temp = estimator_angular_velocity[:, 1].copy()
# estimator_angular_velocity[:, 1] = estimator_angular_velocity[:, 2]
...
det_omega = detector_angular_velocity.T
est_omega = estimator_angular_velocity.T

det_aligned = R @ det_omega  # 仍然是 3xN
est_aligned = R @ est_omega  # 仍然是 3xN

detector_angular_velocity = det_aligned.T
estimator_angular_velocity = est_aligned.T
...
# camera_linear_velocity = np.clip(camera_linear_velocity, -5, 5)
# detector_linear_velocity = np.clip(detector_linear_velocity, -5, 5)
# estimator_linear_velocity = np.clip(estimator_linear_velocity, -5, 5)

camera_linear_velocity = np.clip(camera_linear_velocity, -8, 8)
detector_linear_velocity = np.clip(detector_linear_velocity, -8, 8)
estimator_linear_velocity = np.clip(estimator_linear_velocity, -8, 8)
...
# camera_angular_velocity[np.abs(camera_angular_velocity) > 3] = 0
# detector_angular_velocity[np.abs(detector_angular_velocity) > 3] = 0
# estimator_angular_velocity[np.abs(estimator_angular_velocity) > 3] = 0


gt_data = np.hstack((
    camera_time.reshape(-1, 1),
...
# 保存为 TUM 格式文本文件
output_path = "gt.tum"
np.savetxt(output_path, gt_data, fmt="%.9f")

print(f"✅ 已保存到 {output_path}")

data = np.hstack((
    detector_time.reshape(-1, 1),
    detector_linear_velocity,
    detector_angular_velocity
))

# 保存为 TUM 格式文本文件
output_path = "front.tum"
