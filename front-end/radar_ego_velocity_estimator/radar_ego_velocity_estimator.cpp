  ...
  if (1) // pcl2msgToPcl(radar_scan_msg, *radar_scan)
  {
    ...
    if (valid_targets.size() > 2)
    {
      ...
      else
      {
        ...
        // radar_file << "radar_data = " << radar_data  << std::endl;
  ...
  if (radar_data.rows() >= config_.N_ransac_points)
  {
    ...
    for (uint k = 0; k < ransac_iter_; ++k)
    {
      ...
      if (rtn)
      {
        ...
        // doppler_ransac << "err = " << err << std::endl; 
        // doppler_ransac << "config_.inlier_thresh = " << config_.inlier_thresh << std::endl; 
        for (uint j = 0; j < err.rows(); ++j)
        {
          ...
          {
            // doppler_ransac << "outlier = " << err(j) << std::endl; 
            // doppler_ransac << "b = " << y_all(j) << std::endl; 
            outlier_idx.emplace_back(j);
          }
        }
        // doppler_ransac << std::endl;

        // if too small number of inlier detected, the error is too high, so regard outlier as inlier
        // doppler_ransac << "outlier_idx.size = " << outlier_idx.size() << ", inlier_idx.size = " << inlier_idx.size() << std::endl;
        // doppler_ransac << "outlier_rate = " << float(outlier_idx.size())/(inlier_idx.size()+outlier_idx.size()) << std::endl;
        // if ( float(outlier_idx.size())/(inlier_idx.size()+outlier_idx.size()) > 0.25 ) // 0.05
        // {
        ...
        if (inlier_idx.size() > inlier_idx_best.size())
        {
          ...
          // doppler_ransac << "inlier_idx_best.size = " << inlier_idx_best.size() << std::endl;
        }
                                            ...
                                            bool estimate_sigma)
{
  ...
  double lamda = 1.0;
  HTH += lamda * Matrix::Identity(HTH.rows(), HTH.cols());
