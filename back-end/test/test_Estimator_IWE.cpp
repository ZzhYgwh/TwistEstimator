...
// HAO TODO: 将 cmax框架集成进来
void AngularCallback(const geometry_msgs::TwistStamped::ConstPtr event_angular_ptr);
...
#ifdef USE_IWE_ESTI
    std::vector<TwistVelocity> twist_vec;
#else
...
int main(int argc,char ** argv)
{
...
#ifdef USE_IWE_ESTI
    ros::Subscriber event_angular_sub_ = nh.subscribe<geometry_msgs::TwistStamped::ConstPtr>("/dvs/angular_velocity", 1000, AngularCallback);
#endif
    ...
    twist_vec.reserve(100);
    while(ros::ok())
    {

#ifdef USE_IWE_ESTI
        if (!twist_vec.empty()) // twist.header.seq != -1)
        { 
            // estimator_->Estimate(twist);
            estimator_->Estimate(twist_vec);
            twist_vec.clear();
        }
        // twist.header.seq = -1;
#else
...
#ifdef USE_IWE_ESTI
    TwistVelocity twist;
    twist.timestamp = radar_msgs_ptr->header.stamp.toSec();
    twist.linear_vec << v_r.x(), v_r.y(), v_r.z();
    twist.angular_vec << 0.0, 0.0, 0.0;
    twist_vec.emplace_back(twist);
#else
    geometry_msgs::TwistWithCovarianceStamped twist;
...
{
    TwistVelocity twist;
    twist.timestamp = event_angular_ptr->header.stamp.toSec();
    twist.linear_vec << 0.0, 0.0, 0.0;
    twist.angular_vec << event_angular_ptr->twist.angular.x, 
                        event_angular_ptr->twist.angular.y,
                        event_angular_ptr->twist.angular.z;
    twist_vec.emplace_back(twist);

    // twist = *event_angular_ptr;
}
#endif
